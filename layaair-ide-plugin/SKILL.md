...
- `precompile`: accepts a directory array or an object with `directories`, `minify`, `keepNames`, `define`, and `external`. `minify` and `keepNames` default to `true`. Export replaces the selected source directories in the staged package with UI/Scene bundles under `build~/`; it does not generate Preview/runtime `bundle.js`. Read `references/api-patterns.md` §20 for parameter meanings, examples, dependency packaging, and source-boundary constraints before configuring precompilation.
...
3. **Panel class** must extend `IEditor.EditorPanel`; create an `IEditor.ReactDOM`, mount it in `this._panel`, render JSX, and call `dispose()` in `onDestroy()`. For persistent view state, write to `Editor.workspaceConf.data.getSection(this.panelId)` in synchronous `onSaveStatus()` and restore it in `onStart()`; this is separate from saving business files with `onSave()`. See `references/api-patterns.md` §1.
...
7. **Settings and secrets**: Settings locations are `"project"` (shared), `"local"` (gitignored), `"application"` (global), and `"memory"` (transient). For API keys and credential-bearing HTTP, use UI-process `IEditor.SecretStorage` instead of ordinary settings: `set`, `has`, `remove`, and `request`. There is no plaintext getter; `request` injects authentication in the host. Check `set`'s boolean result for persistent versus memory-only storage. See `references/api-patterns.md` §7.
...
26. **Programmatic diagnostics**: In the UI process, use `IEditor.FrameDebugger.captureFrame()` for one WebGL frame's Spector.js command/state data, and `IEditor.Profiler` for Tracy recording, native `.tracy` export, or JSON CPU-zone/frame statistics. Both are mounted by optional feature packs, share sessions with their editor panels, and do not open UI or file dialogs. See `references/api-patterns.md` §21.
...
- **IFrameDebugger / `IEditor.IFrameDebugger` and IProfiler / `IEditor.IProfiler` declarations** — programmatic WebGL frame capture and Tracy profiling APIs, exposed at runtime as `IEditor.FrameDebugger` and `IEditor.Profiler`
